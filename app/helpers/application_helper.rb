module ApplicationHelper
  ...
  def link_artist_image(artist, options = {})
    filename = "artists/#{artist.name.downcase.gsub(/ /, '_')}.jpg"
    #if !File.exist?(filename)
      #filename = "users/#{artist.user.id}.jpg"
    #end
    link_to image_tag(filename, options.merge({:alt => artist.name, :width => "230px", :height => "200px", :style => 'float: left; clear: left; margin: 0.75em;'})), artist
  end
